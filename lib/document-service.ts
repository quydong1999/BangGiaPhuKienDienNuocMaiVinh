...
export const exportDocumentToExcel = (items: CartItem[], grandTotal: number, docType: 'receipt' | 'quotation', includeVAT: boolean = false) => {
  ...
  for (let R = range.s.r; R <= range.e.r; ++R) {
    for (let C = range.s.c; C <= range.e.c; ++C) {
      ...
      // Header styling
      if (R === 0) {
        cell.s.font = { name: 'Times New Roman', sz: 16, bold: true };
        cell.s.alignment = { horizontal: 'center' };
      } else if (R === 1 || R === 2) {
        cell.s.alignment = { horizontal: 'center' };
      } else if (R === 4) { // Title
        cell.s.font = { name: 'Times New Roman', sz: 20, bold: true };
        cell.s.alignment = { horizontal: 'center' };
      } else if (R >= 6 && R <= 9) { // Customer info
        if (C === 0) cell.s.font = { name: 'Times New Roman', sz: 12, bold: true };
      }
      ...
      if (R >= tableStartRow && R <= tableEndRow) {
        ...
        if (isTableHeader || isTotalRow) {
          ...
          cell.s.font = { name: 'Times New Roman', sz: 12, bold: true };
        }
        if (isTableHeader) {
      ...
      if (R === bangChuRow) {
        if (C === 0) cell.s.font = { name: 'Times New Roman', sz: 12, bold: true };
        if (C === 1) cell.s.font = { name: 'Times New Roman', sz: 12, italic: true };
      }

      // Date & Signatures styling
      if (R === dateRow && C === 4) {
        cell.s.font = { name: 'Times New Roman', sz: 12, italic: true };
        cell.s.alignment = { horizontal: 'center' };
      }
