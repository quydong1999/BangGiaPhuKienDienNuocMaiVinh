...
export default async function AdminDashboardPage() {
  ...
  const stats = [
    { label: 'Tổng doanh thu', value: formatVND(totalAmount), icon: TrendingUp, color: 'text-teal-600 bg-teal-50' },
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat, idx) => (
          <div key={idx} className="bg-white p-6 border border-slate-200 shadow-sm flex items-start justify-between group hover:border-teal-200 transition-all cursor-default">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            ...
            <Link href="/admin/invoices" className="text-xs font-bold text-teal-600 hover:text-teal-700 underline underline-offset-4 decoration-2">Xem tất cả</Link>
          </div>
              ...
              <tbody className="divide-y divide-slate-50 italic">
                {allInvoices.slice(0, 5).map((inv: any) => (
                  <tr key={inv._id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4 text-xs font-black text-teal-700 font-mono tracking-tighter">{inv.invoiceNumber}</td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className={`inline-block w-2 h-2 rounded-full ${inv.status === 'paid' ? 'bg-teal-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-amber-400 shadow-[0_0_8px_rgba(251,191,36,0.5)]'}`} />
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="space-y-6">
          <div className="bg-teal-600 p-6 shadow-xl shadow-teal-200 group">
            <h3 className="text-white font-black text-lg mb-2 italic uppercase">Khuyến nghị</h3>
            <p className="text-teal-100 text-xs font-medium leading-relaxed mb-4">Bạn có <span className="text-white font-black">{pendingInvoices.length}</span> hóa đơn đang chờ thanh toán. Hãy kiểm tra và cập nhật trạng thái ngay!</p>
            <Link href="/admin/invoices?status=pending" className="inline-flex items-center gap-2 bg-white text-teal-700 px-4 py-2 text-xs font-black uppercase tracking-widest hover:bg-teal-50 transition-all active:scale-95">
              Xử lý ngay
              <AlertCircle size={14} />
            </Link>
          </div>
          ...
          <div className="bg-white border border-slate-200 p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button className="p-3 border border-slate-100 rounded-lg text-left hover:border-teal-200 transition-all hover:bg-teal-50/30">
